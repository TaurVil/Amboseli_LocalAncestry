...
vcftools --gzvcf ./test.$coverage.vcf.gz --max-alleles 2 --minQ 30 --positions ../unadmixed_individuals/refpanel.kept.sites --recode --out filt.$coverage
bgzip filt.$coverage.recode.vcf
tabix ./filt.$coverage.recode.vcf.gz

module load java/1.8.0_45-fasrc01

java -jar /data/tunglab/tpv/Programs/GenomeAnalysisTK.jar -T CombineVariants -R $path_genome --variant ./filt.$coverage.recode.vcf.gz --variant ../unadmixed_individuals/refpanel.vcf.gz -o merged.$coverage.vcf.gz -genotypeMergeOptions UNIQUIFY -L 01_targetted_chroms.bed
...
sed '/^#/d' merged.$coverage.forgenolik.vcf | cut -f 1,2,10- | sed -e 's/:/ /g' | sed -e 's/\./999/g' | /data/tunglab/tpv/Programs/LCLAE/filtbaboon1b 70 > genolik.$coverage.genolik


rm filt.$coverage.* ; rm merged.$coverage.forgenolik.vcf ; rm merged.$coverage.vcf.*
