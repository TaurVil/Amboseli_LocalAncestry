...
## get only rows from tmp.forward.bed that are also in tmp.reverse.bed 
awk 'NR==FNR{A[$1];next}$4 in A' tmp2.rev.names human_enhancer.bed > tmp3.bed ; module load R; R
